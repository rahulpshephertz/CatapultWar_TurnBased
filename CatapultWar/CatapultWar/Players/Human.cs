...
namespace CatapultWar
{
    ...
    class Human : Player
    {
        ...
        readonly float maxDragDelta = (new Vector2((float)System.Windows.Application.Current.Host.Content.ActualHeight, (float)System.Windows.Application.Current.Host.Content.ActualWidth)).Length();
                ...
                else if (gestureSample.GestureType == GestureType.DragComplete)
                {
                    ...
                    if (null != firstSample)
                    {
                        ...
                        Catapult.ShotVelocity = MinShotVelocity +
                                                Catapult.ShotStrength * (MaxShotVelocity - MinShotVelocity);
                        if (delta.Length() > 0)
                            Catapult.ShotAngle =MathHelper.Clamp((float)Math.Asin(-delta.Y / delta.Length()),MinShotAngle, MaxShotAngle);
                        else
                            Catapult.ShotAngle = MinShotAngle;
                        if (App.g_isTwoHumanPlayers)
                        {   
                            ...
                            //create the mirror image at remote
                            //Vector2 deltaforRemote = new Vector2(-delta.X, delta.Y);
                            //double velocityForRemote = MinShotVelocity + (deltaforRemote.Length() / maxDragDelta) * (MaxShotVelocity - MinShotVelocity);
                           
                            //if (deltaforRemote.Length() > 0)
                            //    angleForRemote = MathHelper.Clamp((float)Math.Asin(-deltaforRemote.Y / deltaforRemote.Length()), MinShotAngle, MaxShotAngle);
                            //else
                            //    angleForRemote = MinShotAngle;
        ...
        public override void Update(GameTimerEventArgs gameTime)
        {
            ...
            {  
                ...
                else if (msg.Type.Equals("DRAGGING"))
                {
                    ...
                    remoteCurrentSample.X = 800-(float)Convert.ToDouble(msg.X);
        ...
        public override void Draw(GameTimerEventArgs gameTime)
        {
            ...
            //DrawString("Angle:" + Catapult.ShotAngle.ToString() + "\n Velocity:" + Catapult.ShotVelocity.ToString(), new Vector2(200, 0), Color.Red);
