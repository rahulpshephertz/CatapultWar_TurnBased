...
namespace CatapultWar
{
    public partial class GamePage : PhoneApplicationPage
    {
        ...
        DispatcherTimer _TimerForTurn = new DispatcherTimer();
        // Helper members
        bool isDragging;
        #endregion
        public GamePage()
        {
            ...
            timer.Draw += OnDraw;
            _TimerForTurn.Interval = new TimeSpan(0, 0, 0, 1);
            _TimerForTurn.Tick+=_TimerForTurn_Tick;
        }
        ...
        protected override void OnNavigatedTo(NavigationEventArgs e)
        {
            ...
            if (e.NavigationMode == NavigationMode.New)
            {
                ...
                {
                    GlobalContext.notificationListenerObj.AddCallBacks(OpponentLeftTheRoom, OpponentPaused, OpponentResumed,ChangeTurnCallback,onGameStarted);
        ...
        private void OnUpdate(object sender, GameTimerEventArgs e)
        {
            ...
            else if (GlobalContext.IsMyTurn)
            {
                if(changeTurn)
                {
                // Update wind
            ...
            if (isTwoHumanPlayers)
            {
                ...
                {   
                    ...
                    if(GlobalContext.PlayerIsFirstOnAppWarp)
                     ...
                     playerTwo.Score = Convert.ToInt32(GlobalContext.tableProperties["Player1Score"]);
                }
                else
                {
        ...
        public void HandleInput(InputState input)
        {
            ...
            else if (!isPaused && isTwoHumanPlayers && GlobalContext.IsMyTurn && (GlobalContext.joinedUsers.Length == 2) &&
               (playerOne.Catapult.CurrentState == CatapultState.Idle ||
                   playerOne.Catapult.CurrentState == CatapultState.Aiming))
            {
                // Read all available gestures
            ...
            //else if (!isPaused && isTwoHumanPlayers && GlobalContext.IsMyTurn && !GlobalContext.PlayerIsFirstOnAppWarp && (GlobalContext.joinedUsers.Length == 2) &&
            //    (playerTwo.Catapult.CurrentState == CatapultState.Idle ||
            //        playerTwo.Catapult.CurrentState == CatapultState.Aiming))
            //{
            //    // Read all available gestures
        ...
        void DrawHud()
        {
            ...
            else
            {
                ...
                if (isTwoHumanPlayers)
                DrawString(hudFont,GlobalContext.localUsername,
                ...
                if(isTwoHumanPlayers)
                DrawString(hudFont,  GlobalContext.opponentName ,
        ...
        private void ChangeTurnCallback()
        {
            if (!GlobalContext.IsMyTurn)
            {
                _TimerForTurn.Stop();
                _TimeCounter = 0;
                playerOne.isDragging = false;
            }
            else
            {
                _TimeCounter = 20;
        ...
        private void _TimerForTurn_Tick(object sender, EventArgs e)
        {
            if(_TimeCounter>=0)
            _TimeCounter--;
        }
